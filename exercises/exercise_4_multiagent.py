...
def check_routing(state: State) -> list[Send]:
    """Routing function: quyết định gọi agents nào dựa trên nội dung câu hỏi."""
...
Hãy phân tích các vấn đề về privacy, GDPR, data breach (nếu có).
Tập trung: GDPR, CCPA, data protection, quyền của người dùng, nghĩa vụ thông báo vi phạm.
"""
    response = llm.invoke([HumanMessage(content=prompt)])
...
def build_graph() -> StateGraph:
    ...
    # Add nodes — note: check_routing is NOT a node, it's a routing function
    ...
    # law_agent → conditional routing using Send API for parallel dispatch
    graph.add_conditional_edges("law_agent", check_routing, ["tax_agent", "compliance_agent", "privacy_agent", "aggregate_results"])
